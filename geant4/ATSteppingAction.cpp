...
ATSteppingAction::ATSteppingAction()
: G4UserSteppingAction()
{
    fRunManager = (LKG4RunManager *) LKG4RunManager::GetRunManager();
}

ATSteppingAction::ATSteppingAction(LKG4RunManager *man)
    : G4UserSteppingAction(), fRunManager(man)
{
}

void ATSteppingAction::UserSteppingAction(const G4Step* step)
{
    ...
    */
#ifdef TEST_KILL_TRACK
    auto ran = ((ATG4RunManager*) fRunManager) -> fRandomNumber;
    if (preNo==1 && pos.z()>=ran) {
        track -> SetTrackStatus(fStopAndKill);
        fRunManager -> AddTrackVertex(preNo, 9/*i killed the track*/, pos.x(), pos.y(), pos.z(), mom.x(),mom.y(),mom.z(), track->GetKineticEnergy());
    }
#endif
